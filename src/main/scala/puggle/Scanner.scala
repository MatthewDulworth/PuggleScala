...
class Scanner(src: String) {
   ...
   */
  private def peakNext(): Option[Char] =
    if cursor + 1 < src.length then
      Some(src.charAt(cursor + 1)) else None

  /**
   * @param current The character in the source at the cursor.
  ...
  private def getToken(current: Char): Token =
    ...
    private def matchNext(expected: TokenType) = peakNext() match
      case None => false
      case Some(_) => nextChar; true
}