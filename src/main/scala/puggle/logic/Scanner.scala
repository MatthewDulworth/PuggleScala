...
class Scanner(src: String) {
  ...
  @tailrec private def getTokens(list: List[Token] = Nil): List[Token] = {
    ...
    if (token != Token(EOF)) getTokens(result) else result
  }
  ...
  private def nextToken(): Token = nextChar() match
      case Some(c) => scanToken(c) match
        case l: Lexeme => Token(l, line)
        case t: Token => t
      case None => Token(EOF)
   ...
   */
  private def scanToken(c: Char): Token | Lexeme = c match
    case '(' => OPEN_PAREN
    case ')' => CLOSE_PAREN
    case '{' => OPEN_BRACE
    case '}' => CLOSE_BRACE
